...
export function validateTeamMemberOptions(value: string[]): string | undefined {
  ...
  if (unique.size !== value.length) {
    return 'Team member options cannot contain duplicates'
  }
...
export function normalizeBasicInfoFormValues(values: BasicInfoFormValues): {
  owner: string
  email: string
  description: string
  priority: Priority
...
export function normalizeProjectDetailsFormValues(
  ...
  return {
    ...
    options: values.options as TeamMemberOption[],
  }
}
