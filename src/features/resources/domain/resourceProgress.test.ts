...
describe('resourceProgress', () => {
  ...
  it('locks Project Details for incomplete draft Basic Info only', () => {
    const locked = createResource(emptyBasicInfo, emptyProjectDetails)
    const unlockedDraft = createResource(completeBasicInfo, emptyProjectDetails)
    const completed = {
      ...createResource(completeBasicInfo, completeProjectDetails),
      status: 'completed' as const,
    }

    expect(isProjectDetailsLocked(locked)).toBe(true)
    expect(isProjectDetailsLocked(unlockedDraft)).toBe(false)
    expect(isProjectDetailsLocked(completed)).toBe(false)
  })
