...
import type { CompletedResourceDraftsState } from './completedResourceDraft.types'
...
describe('completedResourceDraft helpers', () => {
  ...
  it('removes the resource buffer entry when the final changed module is reverted', () => {
    ...
    expect(stateHasAnyDraftEntries(state)).toBe(false)
  })
  ...
  it('merges effective Basic Info from server data and local override', () => {
    ...
    expect(getEffectiveBasicInfo(serverResource, draft)).toEqual(draft.basicInfo)
  ...
  it('merges effective Project Details from server data and local override', () => {
    ...
    expect(getEffectiveProjectDetails(serverResource, draft)).toEqual(
  ...
  it('builds a full PUT payload from the latest Resource plus current overrides', () => {
    ...
    const payload = buildFullUpdatePayload(serverResource, draft)
  ...
  it('reports only modules that actually differ from server data', () => {
    ...
    expect(getChangedModules(serverResource, draft)).toEqual(['basic-info'])
  })

  it('removes stale overrides that equal refreshed server data', () => {
    const serverResource = completedServerResource()
    let state: CompletedResourceDraftsState = {
      '42': {
        basicInfo: {
          owner: 'New Owner',
          email: 'jane@example.com',
    ...
    state = reconcileDraftInState(state, refreshed)
    expect(state['42']).toBeUndefined()
    expect(reconcileDraftSnapshot(refreshed, {
      basicInfo: {
        owner: 'New Owner',
        email: 'jane@example.com',
        description: 'Server description',
  ...
  it('clears only the requested resource draft', () => {
    const first = completedServerResource()
    const second = createResource({
      ...completedServerResource(),
      resourceId: 7,
    ...
    state = clearDraftFromState(state, 42)
